...
// Function to go back to listEtudiant.php
function goBack() {
    const target = document.getElementById('main-content'); // Target container for dynamic content

    // Check if content is already loaded, if so, don't load again
    if (target.dataset.loaded === 'listEtudiant') {
        return; // Content already loaded, do nothing
    }

    // Load the listEtudiant.php file
    fetch('listEtudiant.php')
        .then(response => {
            if (!response.ok) throw new Error('Failed to load content');
            return response.text();
        })
        .then(data => {
            target.innerHTML = data; // Replace the content with the data from listEtudiant.php
            target.dataset.loaded = 'listEtudiant'; // Mark as loaded
...
function attachEventListeners() {
    const studentDivs = document.querySelectorAll('.listEtudiant'); // Get all student divs
